<template>
  ...
  onMounted(() => {
    ...
    if (storedSelection && weights.value.risk!==null) { // 로컬 스토리지 값이 유효한지 확인
  ...
  const selectOption = (option) => {
    if (selected.value === option) { // 이미 선택된 옵션을 다시 클릭하면 선택 취소
      selected.value = null
      localStorage.removeItem('selectedOption3')  // 선택 취소 시 로컬 스토리지에서 제거
      weights.value.risk = null; // 가중치도 초기화
    } else {
      selected.value = option; // 클릭한 옵션을 활성화
      localStorage.setItem('selectedOption3', option); // 로컬 스토리지에 저장
      // 가중치 반영
      if (option) {
      weights.value.risk = weightMapping.risk[option]; // 'risk'에 해당하는 가중치 저장
    }
  }
};
